...
export function HistorialAsistencia() {
    ...
    const cargarAsistencias = async () => {
      ...
      try {
        const snapshot = await getDocs(
          collection(db, `consejeros/${user.uid}/asistencias`)
        );
        const fechasEnRango = [];
        const conteo = {};
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 bg-white rounded-lg shadow text-gray-800">
      <h2 className="text-2xl font-bold text-scout-secondary mb-2">
            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-gray-100 text-left">
                  ...
                  <th className="border p-3 text-center text-gray-700">
                    Asistencias
                  </th>
                  <th className="border p-3 text-center text-gray-700">
                    % Asistencia
                  ...
                  return (
                    <tr key={protaId} className="hover:bg-gray-50 transition">
                      ...
                      <td className="border p-3 text-center">
                        {resumenData.asistencias}
                      </td>
                      <td className="border p-3 text-center">
                        {resumenData.porcentaje}%
                      </td>
                    </tr>
                  );
                })}
